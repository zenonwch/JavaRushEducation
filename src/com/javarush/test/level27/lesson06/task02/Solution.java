...
public class Solution {
	...
	public static synchronized boolean isNormalLockOrder(final Solution solution, final Object o1, final Object o2) throws Exception {
		...
		while (true) {
			if (t2.getState().equals(Thread.State.TIMED_WAITING)) {
				t1.interrupt();
				t2.interrupt();
				return false;
			}
	...
	public static void main(String[] args) throws Exception {
		...
		new Thread() {
			...
			public void run() {
				try {
					System.out.println("1: " + isNormalLockOrder(solution, o1, o2)); //expected boolean b
				}
				catch (Exception ignored) {
				}
			}
		...
		new Thread() {
			...
			public void run() {
				try {
					System.out.println("2: " + isNormalLockOrder(solution, o2, o1)); //expected boolean !b
				}
				catch (Exception ignored) {
				}
			}
